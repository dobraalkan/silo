...
public class StringUtilities {
	...
	public static boolean isEmpty(Optional<String> maybeString) {
		return OptionalUtilities.ofNullableOptional(maybeString).map(StringUtilities::isEmpty).orElse(true);
	}
	...
	public static boolean isNumericLong(String value) {
		return !isEmpty(value) && parseLong(value, null) != null;
	}
	...
	public static String join(Collection<?> values, String separator) {
		return values.stream().filter(Objects::nonNull).map(Object::toString).collect(Collectors.joining(separator));
	}

	public static <T> String joinUsing(Collection<T> values, Function<T, String> toStringFn, String separator) {
		return values.stream().map(toStringFn).collect(Collectors.joining(separator));
	}

	public static <T> List<T> splitThenMap(String str, String separator, Function<String, T> itemTransformer) {
		ChecksUtilities.checkNotNull(itemTransformer, "itemTransformer cannot be null");
		if (isEmpty(str) || isEmpty(separator)) {
			return Collections.emptyList();
		}

		return Arrays.stream(str.split(separator)).map(itemTransformer).collect(toList());
	}
	...
	public static List<String> tokenise(String value, String regex) {
		ChecksUtilities.checkNotNull(value, "value cannot be null");

		String[] split = value.split(regex);

		return Stream.of(split).filter(s -> !s.isEmpty()).map(String::trim).collect(Collectors.toList());
	}
	...
	public static char firstChar(String str, char dflt) {
		return mkSafe(str).length() > 0 ? str.charAt(0) : dflt;
	}

	public static Optional<Character> firstChar(String str) {
		return mkSafe(str).length() > 0 ? Optional.of(str.charAt(0)) : Optional.empty();
	}
	...
	public static String mkPath(String... segs) {
		ChecksUtilities.checkAll(segs, StringUtilities::notEmpty, "Cannot convert empty or null segments to path");

		return Stream.of(segs).map(Object::toString).collect(Collectors.joining("/")).replaceAll("/+", "/");
	...
	public static String capitalise(String words) {
		...
		List<String> collect = Arrays.stream(words.split("\\s+")).map(w -> {
	...
	public static String sanitizeCharacters(String str) {
		if (str == null) {
			...
			return str.replaceAll("[‐–—-]", "-").replaceAll("[”“]", "\"").replaceAll("[’‘]", "'").replaceAll("​", "")
					.replaceAll("â€œ", "\"").replaceAll("â€", "\"").replaceAll("â€™", "'").replaceAll(" ", " ");
		}
	}
	...
	public static Optional<String> firstNonNull(String... xs) {
		return Stream.of(xs).filter(x -> isDefined(x)).findFirst();
	}
